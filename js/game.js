	...
	function game_()
		{
			...
			this.getGameTime = function(){return GameTime/60;}
			this.setRound = function(round) {
				for (var i = round; i>=0; --i)
					new map_tree(Math.round()*App.w,App.h*1.3+Math.round()*200);
			}
			this.unload = function()
			{
			GraphicsController.unload('overlayGlass');
			}
			this.start = function()
			{
				...
				imgOrbArray[0] = GraphicsController.load('orb_1','gameplay/orb/orb_1')
				imgOrbArray[1] = GraphicsController.load('orb_2','gameplay/orb/orb_2')
				imgOrbArray[2] = GraphicsController.load('orb_3','gameplay/orb/orb_3')
				imgOrbArray[3] = GraphicsController.load('orb_4','gameplay/orb/orb_4')
				imgOrbArray[4] = GraphicsController.load('orb_5','gameplay/orb/orb_5')
				imgOrbArray[5] = GraphicsController.load('orb_6','gameplay/orb/orb_6')
				
				
				this.imgGlass = GraphicsController.load('overlayGlass','overlay/glass');
				GameOverEvent = false;
				GameOver = false;
				GameTime = 0;
				StartDelay = 100;
				...
				GraphicsController.load('recordNew','interface/record');
				GraphicsController.load('recordOld','interface/oldrecord');
				
				
				GraphicsController.load('gmbottom','gameplay/gamemenu/barbottom');
				GraphicsController.load('gmbottom2','gameplay/gamemenu/barbottom2');
				GraphicsController.load('gmbottom3','gameplay/gamemenu/barshade');
				GraphicsController.load('GameMenuPause','gameplay/gamemenu/paused');
				GraphicsController.load('GameMenuReturn','gameplay/gamemenu/return');
				GraphicsController.load('GameMenuSettings','gameplay/gamemenu/settings');
				...
				//App.set_borders(true,true,true);
			}
			this._NewPlayer = function()
			{
				if (typeof Player !== "undefined")
					{
					if (typeof Player.id !== "undefined")
						mapobjectArray.splice(Player.id,1);
					delete Player;
					}
				Player = new player();
				var id = mapobjectArray.push(Player);
				Player.setId(id);
				console.log(id);
				console.log(mapobjectArray[1]);
			}
			...
			this.chkObj = function(obj,layer)
			{
				ret = true;
				if (!obj)
					ret = false;
				//if (obj.layer!=layer)
				//	return false;
				//if (MapOffX+obj.x<-200)
				//	return false;
				//if (MapOffX+obj.x>App.w+200)
				//	return false;
				return ret;
			}
			this.updateObj = function(obj)
			{
				var ph = Player.y+App.h*1.5;
				var ol = mapobjectArray.length-1;
				for(var i =0; i< ol; ++i)
				{
				if (typeof mapobjectArray[i] === "undefined")
					continue;
						if (mapobjectArray[i].type=="Board")
							{
							console.log("board");
							continue;
							}
					if (this.chkObj(mapobjectArray[i]))
					{				
						if (typeof mapobjectArray[i].update === 'function')
							mapobjectArray[i].update(Angle);

						if (mapobjectArray[i].del)
								if (mapobjectArray[i].restore==true)
									{
										if (typeof mapobjectArray[i].init !== 'undefined')
											mapobjectArray[i].init();
										if (mapobjectArray[i].stand)
												mapobjectArray[i].y = ph;
											else
											{
												mapobjectArray[i].x =(Math.random()*(App.w));
												mapobjectArray[i].y = Player.y+App.h*0.9;
												if (typeof mapobjectArray[i].ref !== 'undefined')
													mapobjectArray[i].ref();
											}
										mapobjectArray[i].del = false;
									}
								else
									{
										
										//console.log(i+':Kill:'+mapobjectArray[i].type);
					...
					{
						console.log(i+':Kill:'+mapobjectArray[i]);
						mapobjectArray.splice(i,1);
						continue;
					}
				}
			}
			this.drawMapObjects = function()
			{
				var ph = Player.y+App.h*1.5;
				var ol = mapobjectArray.length-1;
				//mapobjectArray.reverse();
				
				for (var layerI = -1; layerI<=2; ++layerI)
				{
					for(var i = ol; i>0; --i)
						{
						if (this.chkObj(mapobjectArray[i]))
							if (mapobjectArray[i].layer==layerI)
			...
			{
				s = 1.989;
				by = Math.round((App.setHeight+(-MapOffY/s-App.setHeight))%BackgroundH*s-BackgroundH*s);
				by_first = by;
				//App.visuals._color("#FFFFFF");
				//App.visuals.rect(-App.w,0, App.setWidth*3,App.h,1,1,false);
				App.visuals._color("#000000");
				
				for (bx = App.setWidth+(MapOffX/s+App.setWidth*s)%BackgroundW*s-BackgroundW*s; bx < App.setWidth*s+BackgroundW*s; bx+=BackgroundW*s)
				{
					for (by = by_first; by < App.setHeight*s+BackgroundH*s; by+=BackgroundH*s)
					{
						App.visuals.image(BackgroundImage,Math.round(bx-App.setWidth*s/2),Math.round(-by+App.setHeight*s),s+0.003,1,true);
					}
				}
				if (MapOffY<-510)
					{
					App.visuals._color("#0000FF");
					App.visuals.rect((App.setWidth/2)+MapOffX,425+MapOffY, App.setWidth/1.5,25,1,0.3,true);
					}
				...
				for (by = by_first; by < App.setHeight*s+BackgroundH*s; by+=BackgroundH*s)
				{
					if (by%2)
						{
						App.visuals.image(MountianSideL,MapOffX-775+by/BackgroundH*s,Math.round(-by+App.setHeight*s),2,1,true);
						if (Player.x>100)
						App.visuals.image(MountianSideL,MapOffX+App.w*2+275+by/BackgroundH*s,Math.round(-by+App.setHeight*s),2,1,true);
						}
				}
