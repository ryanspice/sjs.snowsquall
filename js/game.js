...
treegenerateClearWave = function()
{
	...
	for(var i = 75;i>0;--i)
		Object.create(sbTree)._init(0-(1*i*i)+Math.random()*50-50,55*i);
    
    
Object.create(sbJump)._init(window.innerWidth/2,window.innerHeight);
Object.create(sbSpeed)._init(window.innerWidth/2,window.innerHeight);
Object.create(sbFlag01)._init(window.innerWidth/2,window.innerHeight);
Object.create(sbFlag01)._init(window.innerWidth/2,window.innerHeight);
Object.create(sbIce)._init(window.innerWidth/2,window.innerHeight);
Object.create(sbSnowman)._init(window.innerWidth/2,window.innerHeight*2);
Object.create(sbLog)._init(window.innerWidth/2,window.innerHeight/3);
Object.create(sbRock)._init(window.innerWidth/2,window.innerHeight/2);
...
Game.prototype = {
	...
	init:function(){
        ...
        this.TreePart = [];
        this.TreePart[0] = this.graphics.load('trees/treeTrunk1');
        this.TreePart[1] = this.graphics.load('trees/treeTrunk1Shadow');
        this.TreePart[2] = this.graphics.load('trees/treeTrunk3');
        this.TreePart[3] = this.graphics.load('trees/treeTrunk3Shadow');
        
        this.SpeedPart = [];
        this.SpeedPart[0] = this.graphics.load('map/Speed/imageGameAssetSpeedpad01Ani01');
        this.SpeedPart[1] = this.graphics.load('map/Speed/imageGameAssetSpeedpad01Ani02');
        this.SpeedPart[2] = this.graphics.load('map/Speed/imageGameAssetSpeedpad01Ani03');
        this.SpeedPart[3] = this.graphics.load('map/Speed/imageGameAssetSpeedpad01Ani04');

                
        this.Flag01Part = [];
        this.Flag01Part[0] = this.graphics.load('map/flag/imageGameAssetFlag01Ani01');
        this.Flag01Part[1] = this.graphics.load('map/flag/imageGameAssetFlag01Ani02');
        this.Flag01Part[2] = this.graphics.load('map/flag/imageGameAssetFlag01Ani03');
        this.Flag01Part[3] = this.graphics.load('map/flag/imageGameAssetFlag01Ani04');
        this.Flag01Part[4] = this.graphics.load('map/flag/imageGameAssetFlag01Ani05');
        this.Flag01Part[5] = this.graphics.load('map/flag/imageGameAssetFlag01Ani06');
        this.Flag01Part[6] = this.graphics.load('map/flag/imageGameAssetFlag01Ani07');
        this.Flag01Part[7] = this.graphics.load('map/flag/imageGameAssetFlag01Ani08');
        this.Flag01Part[8] = this.graphics.load('map/flag/imageGameAssetFlag01Ani09');
        this.Flag01Part[9] = this.graphics.load('map/flag/imageGameAssetFlag01Ani10');
        this.Flag01Part[10] = this.graphics.load('map/flag/imageGameAssetFlag01Ani11');
        this.Flag01Part[11] = this.graphics.load('map/flag/imageGameAssetFlag02');

        this.Rocks = [];
        this.Rocks[0] = this.graphics.load('map/imageGameAssetRockAndSnow01'); 
        this.Rocks[1] = this.graphics.load('map/imageGameAssetRockAndSnow03'); 
        this.Rocks[2] = this.graphics.load('map/imageGameAssetRockDull02'); 
        this.Rocks[3] = this.graphics.load('map/imageGameAssetRockDull03'); 
        
        
        this.Jump = this.graphics.load('map/jump');
        this.Log = this.graphics.load('map/log_1');
        this.snowman = this.graphics.load('map/Snowman');
        this.ice_rocks = this.graphics.load('map/ice_rocks');
        this.ice = this.graphics.load('map/ice_display');
	...
	update:function(){
		...
		this.player.checkTrees();
        
        
        for (var i = mapobjectArray.length-1; i>0; --i)
            mapobjectArray[i]._update();
	...
	//	this.map.x = MapOffX+(width/2*this.app.getCurrent().game.mapscale);
        ...
        var mountain_left_offx = this.map.x*2 - this.main_mountianl.width/2 - width/2;
		...
		this.player.draw_effects();

        for (var i = mapobjectArray.length-1; i>0; --i)
            if (mapobjectArray[i].layer==0)
                mapobjectArray[i]._draw();
